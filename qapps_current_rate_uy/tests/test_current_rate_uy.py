...
class TestCurrentRateUy(TransactionCase):
    ...
    def test_parseo_ui_usa_tcc_directo(self):
        """UI: ISO esperado 'U.I.', rate = TCC directo (no se invierte)."""
        ui = self._crear_moneda('UI')
    ...
    def test_multiples_monedas_en_una_respuesta(self):
        """Una respuesta con USD, EUR y UI crea las tres tasas correctamente."""
        usd = self._crear_moneda('USD')
        eur = self._crear_moneda('EUR')
        ui = self._crear_moneda('UI')
    ...
    def test_exchange_task_actualiza_si_usd_desactualizado(self):
        ...
        # El segundo posicional es last_update == fecha de cierre del servicio.
        _, args, _ = mock_upd.mock_calls[0]
        self.assertEqual(args[1], self.fecha_cierre)
