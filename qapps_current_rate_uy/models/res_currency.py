...
from .res_config_settings import (
    CURRENCY_COTIZACION_MONEDA,
    CURRENCY_EMAIL_NOTIF,
    CURRENCY_SERVICIO_COTIZACION,
...
class ResCurrency(models.Model):
    ...
    def exchange_task(self):
        currency_cotizacion_moneda = (
            self.env["ir.config_parameter"].sudo().get_param(CURRENCY_COTIZACION_MONEDA)
        )
        ...
        currency_servicio_fecha_cierre = (
            ...
            .sudo()
            .get_param(CURRENCY_SERVICIO_FECHA_CIERRE)
        )
        ...
        if len(currency_usd) and (
            not currency_usd[0].date or currency_usd[0].date < last_update
        ):
    ...
    def update_currency_rates(self, active_currency, last_update):
        currency_servicio_cotizacion = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param(CURRENCY_SERVICIO_COTIZACION)
        )
                        ...
                        self.env["res.currency.rate"].create(
                            {
                                ...
                                "rate": float(f"{rate[0][STR_TCC]:.12f}"),
                            }
                        )
                        ...
                        self.env["res.currency.rate"].create(
                            {
                                ...
                                "currency_id": currency.id,
                                "name": rate[0][STR_FECHA],
                                "rate": float(f"{1 / rate[0][STR_TCC]:.12f}"),
                            }
                        )
            else:
    ...
    def send_email_missing_rate(self, currency_name):
        currency_email_notif = (
            self.env["ir.config_parameter"].sudo().get_param(CURRENCY_EMAIL_NOTIF)
        )
        if currency_name:
            subject = (
                f"❌ No se pudo cargar la tasa de cambio de {currency_name} del día"
            )
            body_html = (
                f"<p>No fue posible obtener la tasa de cambio de <strong>{currency_name}</strong> "
            ...
            body_html = (
                "<p>No fue posible obtener la tasa de cambio correspondiente al día de hoy.<br/>"
                "Por favor, revise la configuración o ingrese la tasa manualmente para evitar "
                "inconvenientes.<br/><br/>"
                "Saludos,</p>"
            )
