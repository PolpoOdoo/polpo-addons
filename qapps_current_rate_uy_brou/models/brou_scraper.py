...
class ResCurrencyRate(models.Model):
    ...
    def fetch_brou_rates(self):
        ...
        try:
            ...
            currency_dol = self.env["res.currency"].search(
                [("name", "=", "DOL")], limit=1
            )
            currency_doc = self.env["res.currency"].search(
                [("name", "=", "DOC")], limit=1
            )
            ...
            dolar_row = next(
                (
                    ...
                    if row.find("p", class_="moneda")
                    and row.find("p", class_="moneda").text.strip() == "Dólar"
                ),
                None,
            )
            ...
            if dolar_row:
                ...
                compra = (
                    cols[2].find("p", class_="valor").text.strip().replace(",", ".")
                    if cols[2].find("p", class_="valor")
                    else "0.0"
                )
                venta = (
                    cols[4].find("p", class_="valor").text.strip().replace(",", ".")
                    if cols[4].find("p", class_="valor")
                    else "0.0"
                )
